...
class Yolov3Detection():
    def __init__(self,layer_id):
        ...
        parser.add_argument("-c", "--config",
                            default='configs/dcn/zzj_' + str(layer_id) + '.yml',
        ...
        # 仅读取文件路径
        test_images = self.get_test_images(FLAGS.infer_dir, FLAGS.infer_img)
        dataset.set_images(test_images)

        #scope = fluid.Scope()

        #with fluid.scope_guard(scope=scope):
    ...
    def decode_im(self,frame, sample):
        ...
        return sample
    def single_image_tensor_detect(self,im):
        ...
        while count < 100000:
            ...
            if cap.isOpened():
                ...
                if not ret or frame is None:
                    continue
                tic = time.time()
                res, bbox_results = self.single_image_tensor_detect(frame)
    ...
    def get_save_image_name(self,output_dir, image_path):
        """
